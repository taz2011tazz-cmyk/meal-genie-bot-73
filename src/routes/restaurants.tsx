  ...
  return (
    <div className="marketplace-dark min-h-screen bg-background pb-36 text-foreground">
      ...
      <main className="mx-auto max-w-3xl space-y-8 pt-5">
        <section aria-label="Food categories"><div className="no-scrollbar flex snap-x gap-4 overflow-x-auto px-4 pb-1">{DISCOVERY_CATEGORIES.slice(0, 8).map((item, index) => <Button key={item.key} variant="ghost" className="h-auto w-20 shrink-0 snap-start flex-col gap-2 rounded-2xl p-0" onClick={() => setCategory(category === item.key ? null : item.key)} aria-pressed={category === item.key}><span className={cn("grid h-16 w-16 place-items-center overflow-hidden rounded-full border text-3xl transition-transform active:scale-95", category === item.key ? "border-primary bg-primary/15 ring-2 ring-primary/30" : "border-border bg-card")}><img src={FALLBACKS[index % FALLBACKS.length]} alt="" className="h-full w-full object-cover" /></span><span className={cn("w-full whitespace-normal text-center text-xs", category === item.key ? "font-bold text-primary" : "text-foreground")}>{item.label}</span></Button>)}</div></section>
        ...
        {isLoading ? <MarketplaceSkeleton /> : isError ? <div className="mx-4 rounded-2xl border border-destructive/40 bg-card p-6 text-center"><Store className="mx-auto h-8 w-8 text-muted-foreground" /><h2 className="mt-3 font-sans text-lg font-bold">Restaurants didn't load</h2><p className="mt-1 text-sm text-muted-foreground">Check your connection and try again.</p><Button className="mt-4 rounded-full" onClick={() => refetch()} disabled={isFetching}><RotateCcw />Try again</Button></div> : rows.length === 0 ? <EmptyResults clear={() => { setSearch(""); setCategory(null); setFilter("all"); }} /> : <>
          ...
          <RestaurantSection title="Recommended for you" rows={rows} promotions={promotions} isFavourite={isFavourite} toggle={toggle} />
...
function PromoCarousel({ banners, restaurants, slide, setSlide, carouselRef }: { banners: FeedPromotion[]; restaurants: Map<string, FeedRestaurant>; slide: number; setSlide: (value: number) => void; carouselRef: React.RefObject<HTMLDivElement | null> }) {
  return <section aria-label="Offers"><div ref={carouselRef} className="no-scrollbar flex snap-x snap-mandatory overflow-x-auto" onScroll={(event) => { const width = event.currentTarget.clientWidth; if (width) setSlide(Math.round(event.currentTarget.scrollLeft / width)); }}>{banners.map((promo, index) => { const restaurant = restaurants.get(promo.restaurant_id); return <article key={promo.id} className="w-full shrink-0 snap-center px-4"><div className="relative h-52 overflow-hidden rounded-3xl border border-border bg-card"><img src={restaurant?.cover_image_url || FALLBACKS[(index + 3) % FALLBACKS.length]} alt="Promotional food" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-[linear-gradient(90deg,oklch(0.08_0.01_150/0.9),oklch(0.08_0.01_150/0.15))]" /><div className="absolute inset-y-0 left-0 flex w-3/4 flex-col justify-center p-6"><span className="mb-2 text-xs font-bold uppercase text-primary">{promoHeadline(promo, restaurant?.currency)}</span><h2 className="font-sans text-2xl font-black text-white">{promo.title}</h2><p className="mt-1 line-clamp-2 text-sm text-white/75">{promo.description || restaurant?.name || "Fresh flavour, delivered."}</p>{restaurant && <Button asChild size="sm" className="mt-4 w-fit rounded-full"><Link to="/restaurant/$slug" params={{ slug: restaurant.slug }}>Order now <ChevronRight /></Link></Button>}</div></div></article>; })}</div><div className="mt-3 flex justify-center gap-1.5">{banners.map((banner,index) => <Button key={banner.id} variant="ghost" size="icon" className="h-5 w-5 rounded-full p-0" aria-label={`Show offer ${index + 1}`} onClick={() => { setSlide(index); carouselRef.current?.scrollTo({ left: carouselRef.current.clientWidth * index, behavior: "smooth" }); }}><span className={cn("h-1.5 rounded-full transition-all", slide === index ? "w-5 bg-primary" : "w-1.5 bg-muted-foreground/40")} /></Button>)}</div></section>;
}
