  ...
  return (
    <div className="marketplace-dark min-h-screen bg-background pb-36 text-foreground">
      ...
      <main className="mx-auto max-w-3xl space-y-8 pt-5">
        <section aria-label="Food categories"><div className="no-scrollbar flex snap-x gap-4 overflow-x-auto px-4 pb-1">{DISCOVERY_CATEGORIES.slice(0, 8).map((item) => <Button key={item.key} variant="ghost" className="h-auto w-20 shrink-0 snap-start flex-col gap-2 rounded-2xl p-0" onClick={() => setCategory(category === item.key ? null : item.key)} aria-pressed={category === item.key}><span className={cn("grid h-16 w-16 place-items-center rounded-full border bg-card text-3xl transition-transform active:scale-95", category === item.key ? "border-primary bg-primary/15 ring-2 ring-primary/30" : "border-border")}>{item.emoji}</span><span className={cn("w-full whitespace-normal text-center text-xs", category === item.key ? "font-bold text-primary" : "text-foreground")}>{item.label}</span></Button>)}</div></section>
